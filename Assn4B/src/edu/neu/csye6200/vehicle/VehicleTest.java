...
public class VehicleTest {
	...
	public static void run()  {
		...
		File f = new File("vehicles");
		f.mkdir();
		RegistryIO registryIO = new RegistryIO();
        registryIO.save(registryDetails, f.getAbsolutePath());
        
        ArrayList<Vehicle> veh = registryIO.load(registryDetails, f.getAbsolutePath());
        if (veh != null) {
            System.out.println("\nPrinting deserialize objects\n");

            for (Vehicle eachVehicle : veh) {
