...
public class AuctionServer
{
		...
		public int submitItem(String sellerName, String itemName, int lowestBiddingPrice, int biddingDurationMs)
	{
		...
		//   Don't forget to increment the number of things the seller has currently listed.
				if ((lowestBiddingPrice * 1) ==  Math.abs(lowestBiddingPrice) ) {
				if (! (itemsUpForBidding.size() > serverCapacity)) {
			if (! (itemsPerBuyer.size() > maxBidCount)) {
				lastListingID += 1; 
...
public List<Item> getItems()
	...
	public boolean submitBid(String bidderName, int listingID, int biddingAmount)
	{
		...
		//   Put your bid in place---
			...
			if (itemsAndIDs.containsKey(listingID) == true && (itemsPerBuyer.get(bidderName) < maxBidCount) && (highestBids.get(listingID)<=biddingAmount)) {
	...
	public int checkBidStatus(String bidderName, int listingID)
	{
		...
		//     Update the number of open bids for this seller
			Item to_ch = itemsAndIDs.get(listingID);
			if (to_ch.seller() == bidderName && to_ch.biddingOpen() == false && highestBidders.get(listingID).equals(bidderName)) {
				itemsUpForBidding.remove(to_ch);
				itemsAndIDs.remove(listingID);
				highestBids.remove(listingID);
				highestBidders.remove(listingID);
				itemsPerSeller.put(to_ch.seller(), (itemsPerSeller.get(to_ch.seller())-1));
				itemsPerBuyer.put(bidderName, (itemsPerBuyer.get(bidderName)-1));
				return 1;
			} else if (to_ch.biddingOpen() == true){
				return 2;
			}
			else {
				itemsUpForBidding.remove(to_ch);
	...
	public Boolean itemUnbid(int listingID)
	{
		a.lock();
		try { 
		Item chk = itemsAndIDs.get(listingID);
		if (chk.lowestBiddingPrice() != highestBids.get(listingID)){
		return false;
		} else {
		return true;	
		}
